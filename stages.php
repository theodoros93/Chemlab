...
$table = [
	'header' => 
	['A/A', 'Raw Material', 'CAS', 'sp.gr.', 'MW', 'price/kg', 'unit<br/>(kg,g,L,ml)', 'qty', 'cc/kg INPUT', 'cc/kg OUTPUT', 'cc/kg API', 'actual cc/kg API', 'cost', 'mol', 'mol ratio', 'contribution %', 'Action'],
	'footer' => 
...
<div class="headertext"><h3>Welcome '; echo $_SESSION["username"]; echo '</h3><h3>'.$title.'</h3></div>
<form method="post">
		...
		<thead>
			<tr>';
			 foreach($table['header'] as $head): 
				echo '<th data-text="'.strtolower($head).'">'.$head.'</th>';
			 endforeach; 
			echo '</tr>
		</thead>
		<tbody>
			<tr data-stage="1"><td class="stage-text" colspan="17">Stage 1</td></tr>
			<tr class="prefooter">';
			 foreach($table['footer'] as $key): 
				echo '<td data-text="'.strtolower($key).'">'.$key.'</td>';
			 endforeach; 
			echo '</tr>
			<tr class="footer" data-stage="1">';
			 foreach($table['footer'] as $key): 
				echo '<td>
					<input '.(empty($key) ? 'readonly' : 'class="form-control" data-key="'.strtolower($key).'"').'
					type="'.(strtolower($key) == 'output' ? 'text' : 'number').'" />
				</td>';
			 endforeach; 
			echo '</tr>
		</tbody>
	...
	<hr/>
	 ...
	 if ( !empty($materials) ): 
		echo '<div class="col-lg-4">
			...
			<input type="button" id="add-material2-btn" class="btn btn-success" value="Add" />
		</div>';
